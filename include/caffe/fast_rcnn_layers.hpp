...
template<typename Dtype>
class InterpolateLayer : public Layer<Dtype> {
 public:
  explicit InterpolateLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "Interpolate"; }
 ...
 protected:
  ...
  virtual void Forward_gpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void bilinear_interpolate(const Dtype* bottom_data, const int height, 
