...
template<typename Dtype>
__global__ void ROIPoolingBackward(const int nthreads, const Dtype* top_diff,
    const int* argmax_data, const int num_rois, const Dtype spatial_scale,
    const int channels, const int height, const int width,
    const int pooled_height, const int pooled_width, Dtype* bottom_diff,
    const Dtype* bottom_rois){
    CUDA_KERNEL_LOOP(index, nthreads){
        // (n,c,h,w) coordinate in top feature map
      int pw = index % pooled_width;
      int ph = (index / pooled_width) % pooled_height;
      ...
      const int maxidx = argmax_data[index];

      bottom_diff[offset + maxidx] += is_empty ? 0 : top_diff[index];
    }
}
...
void ROIPoolingLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  // NOLINT_NEXT_LINE(whitespace/operators)
  ROIPoolingBackward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
      top[0]->count(), top_diff, argmax_data, top[0]->num(), spatial_scale_, channels_,
