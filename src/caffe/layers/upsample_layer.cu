...
template <typename Dtype>
__global__ void UpsampleForward(const int nthreads, const Dtype* bottom_data, const int channels, 
            const int height, const int width,const int pooled_height, const int pooled_width,
            const Dtype spatial_scale,Dtype* top_data){
  CUDA_KERNEL_LOOP(index, nthreads){
    ...
    Dtype index_h = static_cast<Dtype>(ph/spatial_scale);
    Dtype index_w = static_cast<Dtype>(pw/spatial_scale);
...
template <typename Dtype>
void UpsamplePoolingLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
             const vector<Blob<Dtype>*>& top) {
  const Dtype* bottom_data = bottom[0]->gpu_data();
  Dtype* top_data = top[0]->mutable_gpu_data();
  int count = top[0]->count();
  UpsampleForward<Dtype> << <CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS >> >
    (count, bottom_data, channels_, height_, width_, pooled_height_,pooled_width_,spatial_scale_,top_data);
...
template <typename Dtype>
__global__ void UpsampleBackwardFeature(const int nthreads, const Dtype* top_diff,
          const int channels,const int height, const int width, const int pooled_height,
          const int pooled_width,const Dtype spatial_scale,Dtype* bottom_diff){
  CUDA_KERNEL_LOOP(index, nthreads){
...
template <typename Dtype>
void UpsamplePoolingLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if(propagate_down[0]){
    UpsampleBackwardFeature<Dtype> << <CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS >> >
    (count, top_diff, channels_, height_, width_, pooled_height_,pooled_width_,spatial_scale_,bottom_diff);
  }
  CUDA_POST_KERNEL_CHECK;
}
INSTANTIATE_LAYER_GPU_FUNCS(UpsamplePoolingLayer);
}