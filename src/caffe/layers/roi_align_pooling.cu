...
__device__ void bilinear_interpolate(const Dtype* bottom_data, const int height, 
...
__global__ void ROIAlignForward(const int nthreads, const Dtype* bottom_data,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    //Find the maximum value in the four corners of each ROI bin and record its cordinate
    for(int i = 0;i<2;++i){
      for(int j = 0;j<2;++j){
    ...
    argmax_data_h[index] = is_empty ? -1 : maxidx_h;
    argmax_data_w[index] = is_empty ? -1 : maxidx_w;
  }
}
template <typename Dtype>
...
__global__ void ROIAlignBackWardFeature(
  ...
  CUDA_KERNEL_LOOP(index, nthreads){
    //(n,c,ph,pw) in the top diff feature map
    int pw = index % pooled_width;
    int ph = (index / pooled_width) % pooled_height;
    int c = (index / pooled_width / pooled_height) % channels;
    int n = index / pooled_width / pooled_height / channels;
    ...
    Dtype offset_argmax_h = argmax_data_h[index];
    Dtype offset_argmax_w = argmax_data_w[index];

    Dtype weight_upper_left,weight_upper_right,weight_lower_left,weight_lower_right;

    weight_upper_left = get_feature_gradient(offset_argmax_h,offset_argmax_w,(int)offset_argmax_h,(int)offset_argmax_w,height,width);
    weight_upper_right = get_feature_gradient(offset_argmax_h,offset_argmax_w,(int)offset_argmax_h,(int)offset_argmax_w+1,height,width);
    weight_lower_left = get_feature_gradient(offset_argmax_h,offset_argmax_w,(int)offset_argmax_h+1,(int)offset_argmax_w,height,width);
    weight_lower_right = get_feature_gradient(offset_argmax_h,offset_argmax_w,(int)offset_argmax_h+1,(int)offset_argmax_w+1,height,width);

    const int offset_upper_left =  offset + ((int)offset_argmax_h) * width + (int)offset_argmax_w;
    const int offset_upper_right = offset + ((int)offset_argmax_h) * width + (int)offset_argmax_w + 1;
    const int offset_lower_left = offset + ((int)offset_argmax_h + 1) * width +(int)offset_argmax_w;
    const int offset_lower_right = offset + ((int)offset_argmax_h + 1) * width +(int)offset_argmax_w + 1;

    bottom_diff[offset_upper_left] += (is_empty) ? 0.:  weight_upper_left * top_diff[index];
    bottom_diff[offset_upper_right] += (is_empty) ? 0.: weight_upper_right * top_diff[index];
    bottom_diff[offset_lower_left] += (is_empty) ? 0.: weight_lower_left * top_diff[index];
    bottom_diff[offset_lower_right] += (is_empty) ? 0.: weight_lower_right * top_diff[index];   
  }
}
template <typename Dtype>
__device__ Dtype get_coordinate_gradient(int coordinate_index, Dtype h, Dtype w, 
  ...
  Dtype map_ratio_h = static_cast<Dtype>(2 * oh + 1) / static_cast<Dtype>(2 * pooled_height);
  Dtype map_ratio_w = static_cast<Dtype>(2 * ow + 1) / static_cast<Dtype>(2 *pooled_width);
...
template <typename Dtype>
__global__ void ROIAlignBackwardCoordinate(const int nthreads, const int pooled_width, const int pooled_height, 
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    Dtype roi_start_w = static_cast<Dtype>(offset_bottom_rois[1] * spatial_scale);
    Dtype roi_start_h = static_cast<Dtype>(offset_bottom_rois[2] * spatial_scale);
    Dtype roi_end_w = static_cast<Dtype>(offset_bottom_rois[3] * spatial_scale);
    Dtype roi_end_h = static_cast<Dtype>(offset_bottom_rois[4] * spatial_scale);
...
void ROIAlignLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    ROIAlignBackWardFeature<Dtype> <<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS >>>
      (count, top_diff, argmax_data_h, argmax_data_w, spatial_scale_, channels_,
  ...
  if (propagate_down[1]) {
    ...
    ROIAlignBackwardCoordinate<Dtype><<<CAFFE_GET_BLOCKS(buffer_count), CAFFE_CUDA_NUM_THREADS>>>(
